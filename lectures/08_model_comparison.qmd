...
`modelSelection` requires `sparseMatrixStats`. Install it as follows
```{r, eval=FALSE}
library(BiocManager)
...
`bestBIC` in package `modelSelection` searches all possible models (if one can enumerate them)

```{r}
email= transform(email, spam= as.numeric(as.character(spam))) #convert outcome to numeric 0/1 values
fitall= bestBIC(spam ~ to_multiple+cc+sent_email+image+attach+dollar+winner, data=email, family="binomial", verbose=FALSE)
fitall
```
