...
**Example.** $y_i = x_{1i}^T \beta_1 + x_{2i}^T \beta_2 + \epsilon_i$
...
$$
E(y_i) = \beta_0 + \beta_1 \mbox{manhattan_i} + \beta_2 \mbox{lSqFt0_i} + \beta_3 \mbox{manhattan}_i \times \mbox{lSqFt0_i}
$$
...
Testing $(\beta_1,\beta_2)=(0,0)$ not the same as testing $\beta_1=0$ and $\beta_2=0$. Even if the latter are both non-significant, the former may be
...
2. Suppose you classify as spam whenever $\hat{\pi}_i > 0.5$. Use `cv.glm` to obtain a cross-validated estimate of the proportion of miss-classifications by `fit1` and `fit3`. Whic of the 2 models is best?
