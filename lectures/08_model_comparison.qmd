...
Fit the 4 models. Pass our `cost_loglik_logistic` to `cv.glm`

`(x + y)^2` adds main effects + interactions, i.e. same as (`x + y + x:y`)
...
To compare the models `fit1` and `fit3` fitted to the email data:
...
A way to resolve MLE pathological behavior is to use a penalized likelihood method

Essentially, it maximizes the log-likelihood plus a penalty for having estimates far from 0

- Prevents infinite estimates 

- Improves finite $n$ bias & mean squared error

This is implemented in package `brglm2`

```{r}
f= formula(spam ~ to_multiple + sent_email + image + attach + dollar + winner)
fitpen= brglm(f, data=email, family=binomial())
```

---

The standard error for `sent_email` doesn't explode anymore
...
```{r}
cbind(coef(fitpen), confint(fitpen))
```
...
A simple type of MCMC algorithm is Gibbs sampling

**Gibbs sampler.** Initialize $(z_1^{(0)},\ldots,z_p^{(0)})$. 

For $b=1,\ldots,B$, update $z_j^{(b)}$ from its conditional distrib. given the current values of $(z_1,\ldots,z_{j-1},z_{j+1},\ldots,z_p)$. That is


$$
z_j^{(b)} \sim \pi(z_j \mid z_1^{(b)},\ldots,z_{j-1}^{(b)},z_{j+1}^{(b-1)},\ldots,z_p^{(b-1)})
$$

## Gibbs model search

Denote a model by $\gamma=(\gamma_1,\ldots,\gamma_p)$, where $\gamma_j \in \{0,1\}$ indicates whether parameter $j$ is in the model

**Example.** With $p=2$ continuous covariates we have
...
1. Sample $\gamma_1^{(b)} \sim \pi(\gamma_1 \mid \gamma_2^{(b-1)})$

2. Sample $\gamma_2^{(b)} \sim \pi(\gamma_2 \mid \gamma_2^{(b)})$

## Gibbs model search

Since we defined
$$
\pi(\gamma)= \frac{e^{-BIC_\gamma/2}}{\sum_{\gamma'} e^{-BIC_{\gamma'}/2}}
$$
simple algebra shows that
$$
\pi(\gamma_j=1 \mid \ldots,\gamma_{j-1},\gamma_{j+1},\ldots)=
\frac{\pi(\ldots, \gamma_{j-1}, 1, \gamma_{j+1},\ldots)}{\pi(\ldots, \gamma_{j-1}, 0, \gamma_{j+1},\ldots) + \pi(\ldots, \gamma_{j-1}, 1, \gamma_{j+1},\ldots)}
$$

That is, we can sample $\gamma_j$ given the rest by comparing only 2 models
