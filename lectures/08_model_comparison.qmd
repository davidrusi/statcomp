...
These results assume that the model is correct, e.g. data truly follow a Normal linear regression where
...
$$
E(y_i) = \beta_0 + \beta_1 \mbox{manhattan_i} + \beta_2 \mbox{lSqFt_i} + \beta_3 \mbox{manhattan}_i \times \mbox{lSqFt_i}
$$
...
$$
E(y_i) = \beta_0 + \beta_1 \mbox{manhattan_i} + \beta_2 \mbox{lSqFt_i} + \beta_3 \mbox{manhattan}_i \times \mbox{lSqFt0_i}
$$
...
Estimating and forecasting tend to be more liberal, e.g. MLE $\hat{\beta}_1$ has good properties as $n$ grows. This said, adding unnecessary parameters to a model can hurt predictions
...
Fit polynomials of degree 0, 1, 3 and 9
...
Degree 3 fit predicts better **future data** than degree 9

```{r}
#| code-fold: true
xnew= seq(0,1, length=1000)
ynew= sin(2*pi*xnew) + rnorm(length(xnew),0,sd=0.2)
xpolnew= matrix(NA, nrow= length(xnew), ncol=10)
for (j in 1:10) xpolnew[,j]= xnew^(j-1)

err1new= ynew - b1[1] * xnew*b1[2]
err3new= ynew - xpolnew[,1:4] %*% matrix(b3, ncol=1)
err9new= ynew - xpolnew %*% matrix(b9, ncol=1)
#sqrt(c(mean(err1new^2), mean(err3new^2), mean(err9new^2))) #root MSE

errnew= c(err1new, err3new, err9new)
degree= rep(c(1,3,9), each=length(ynew))
df= tibble(errnew, degree)

ggplot(df, aes(x=factor(degree), y=errnew)) +
  geom_abline(slope=0, intercept=0, col='gray') +
  geom_violin() + 
  labs(x='Degree of polynomial', y='Prediction error in new data')
```

## Over-fitting

Over-fitting: the prediction accuracy in our **training data** does not generalize to **new data**
...
## CV for non-continuous outcomes
...
`cv.glm` lets us specify any loss function. Let us use the negative log-likelihood, implemented in `cost_loglik_logistic` at `routines.R`
...
```{r, warning=FALSE}
fit1= glm(spam ~ to_multiple + from + cc, data=email, family=binomial())
fit1cv= cv.glm(email, fit1, cost=cost_loglik_logistic, K=10)

fit2= glm(spam ~ (to_multiple + from + cc)^2, data=email, family=binomial())
fit2cv= cv.glm(email, fit2, cost=cost_loglik_logistic, K=10)

fit3= glm(spam ~ to_multiple + from + cc + sent_email + image + attach + dollar + winner, data=email, family=binomial())
fit3cv= cv.glm(email, fit3, cost=cost_loglik_logistic, K=10)

fit4= glm(spam ~ (to_multiple + from + cc + sent_email + image + attach + dollar + winner)^2, data=email, family=binomial())
fit4cv= cv.glm(email, fit4, cost=cost_loglik_logistic, K=10)
...
The % of miss-classified observations is another natural loss

Suppose that one predicts $\hat{y}_i=1$ when $\hat{\pi}_i > 0.5$. Function `cost_misclass` at `routines.R` implements this loss
...
We first obtain said % in an in-sample fashion

```{r}
pi1= predict(fit1, type='response')
pi3= predict(fit3, type='response')

loss.insample= c(cost_misclass(email$spam, pi1), cost_misclass(email$spam, pi3))
names(loss.insample)= c('model 1', 'model 3')
loss.insample
```
...
Compare the models `fit1` and `fit3` fitted to the email data

1. Test the null hypothesis $H_0$ that all parameters added by `fit3` are truly 0. Do you reject $H_0$?

2. Suppose you classify as spam whenever $\hat{\pi}_i > 0.5$. Use `cv.glm` to obtain a cross-validated estimate of the proportion of miss-classifications by `fit1` and `fit3`
...
Suppose we take $\hat{y}_i$ when $\hat{\pi}_i > t$ for some $t$ 
...
emailtrain= email[sel,]
emailtest= email[-sel,]

fit1train= glm(spam ~ to_multiple + from + cc, data=emailtrain, family=binomial())
fit3train= glm(spam ~ to_multiple + from + cc + sent_email + image + attach + dollar + winner, data=emailtrain, family=binomial())
...
In the ROC, Model 3 seems much better than Model 1

Recall that the % miss-classifications using $t=0.5$ were similar. Let's check how is that possible


```{r}
table(pi1test > 0.5, emailtest$spam)
```


```{r}
table(pi3test > 0.5, emailtest$spam)
```

$t=0.5$ corresponds to sensitivity $\approx 0$ and specificity $\approx 1$
...
Model selection strategy: evaluate BIC for all candidate models $M_1,\ldots,M_K$, choose the one with lowest BIC (analogoustly for AIC)
...
allbics= bestBIC(spam ~ to_multiple+from+cc+sent_email+image+attach+dollar+winner, data=email, family="binomial")
allbics
```

---

We can fit the model with best BIC. We get a warning for fitted probabilities near 0 or 1.

```{r}
f= formula(spam ~ to_multiple + from + sent_email + image + attach + dollar)
fit.bestbic= glm(f, data=email, family=binomial())
```

Some standard errors are very large, causing problems when getting confidence intervals

```{r}
coefSummary(fit.bestbic)
```

The issue is that `from` is almost a constant. 

```{r}
table(email$from)
```

Let's drop it. Numerical issues improve, but some remain for `sent_email`, due to some estimated probabilities being near 0 or 1

```{r}
f= formula(spam ~ to_multiple + sent_email + image + attach + dollar)
fit.bestbic= glm(f, data=email, family=binomial())
coefSummary(fit.bestbic)
```
...
We discuss **Markov Chain Monte Carlo** methods for model search


## Markov Chain Monte Carlo

MCMC is an ample family of algorithms to sample from (pretty much) any probability distribution
...
Note: the /2 comes from a Bayesian interpretation of the BIC



- Trick 2: sample from $\pi(M_k)$. The higher $\pi(M_k)$, the higher the probability of visiting this model
