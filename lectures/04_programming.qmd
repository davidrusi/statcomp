...
```{r}
#Find x s.t. f(x)=0. Assuming f(low)<0, f(up)>0, single zero
...
Function `summarizeContVars` returns (mean, sd, Q1, Q3) for all columns in a dataset (i.e. puts the code in the previous slide in a function)

```{r}
summarizeContVars= function(x) {
  if (!all_numeric(x)) stop("All columns in x must be numeric")
  x_sum= map_df(x, describe_contvar)
  x_sum$varname= names(x)
