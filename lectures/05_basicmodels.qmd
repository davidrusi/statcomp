...
$$E(\log_2 \mbox{price}_i \mid \mbox{fair}_i=1)= \beta_0 + \ldots $$

$$
E(\log_2 \mbox{price}_i \mid \mbox{good}_i=1)= \beta_0 + \beta_1 + \ldots
$$
...
Estimates and CIs in the log2-scale
...
```{r}
#dplyr::select ensures using function select from package dplyr
...
contrasts(gm3$country0) = contr.sum(unique(gm3$country0)) #tell contr.sum the factor levels
```
