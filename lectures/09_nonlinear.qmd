...
Obtaining splines is easy. Consider
...
- Many tuning parameters: number of nodes, layers, regularization, dropout. Typically set via train/test data split
...
Fit the model. Epochs: number of passes through data. 10% of training data to evaluate out-of-sample performance at each epoch
...
data= mutate(data, Survived= factor(data$Survived), female= ifelse(Sex=='female', 1, 0)) |>
  select(-Age)
```
...
select(importance_frame, variable, gini_decrease, times_a_root, mean_min_depth) |> 
...
data= mutate(data, SurvivedTxt= ifelse(Survived==1,'Yes','No'))
```
...
```{r}
testing= mutate(testing, ypred.logreg= factor(ifelse(prob.logreg > 0.5, 1, 0)))
