...
```{r}
model.logreg= caret::train(Survived ~ Pclass + female + SibSp + Embarked + Parch + Fare, data=data, family='binomial', method='glm')
model.logreg
```
...
```{r}
#library(RSNNS)
model.gam= caret::train(Survived ~ Pclass + female + SibSp + Embarked + Parch + Fare, data=data, family='binomial', method='gam')
model.gam
```
...
```{r}
model.nnet= caret::train(Survived ~ Pclass + female + SibSp + Embarked + Parch + Fare, data=data, family='binomial', method='nnet', trace=FALSE)
#tuneGrid= data.frame(layer1= c(50,100), layer2= c(50,100), layer3=1)
#trControl=trainControl(method='cv',number=5)
#model.nnet= caret::train(Survived ~ Pclass + female + SibSp + Embarked + Parch + Fare, data=data, family='binomial', method='mlpML', tuneGrid=tuneGrid, trControl=trControl)
model.nnet
```
...
2. Compare out-of-sample accuracy vs. logistic regr. and neural nets (80% training, 20% test)

3. Plot out-of-sample AUC curve for the 3 methods


Getting started (data pre-processing)
