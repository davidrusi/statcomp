...
Or, use `print` and combined with functions like `png` or `pdf`
...
Adds animation layers to your ggplot. The main layer are transitions, specifying how data should be spread as the animation progresses

- `transition_states()`: transition according to a discrete variable

- `transition_time()`: transition according to time


Other layers

- `ease_aes()`: how aesthetics should be eased during transitions (linear, gain momentum over time...)
...
The full list of options is [here](https://gganimate.com/reference/index.html)
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
  ...
  scale_size(range = c(2, 12)) +
  scale_x_log10() +
#  geom_text(aes(label = country)) + #to add country labels
...
transition_time treats time as continuous, e.g. adding intermediate frames between years

transition_states evolves in a more "discrete" manner
...
You can reveal new points in an increasing fashion. For example, one may create this plot.
